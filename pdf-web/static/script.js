document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupDownloadLinks(filesData) {
        ...
        downloadButtons.forEach(button => {
            ...
            if (filesData[fileId]) {
                ...
                // 파일 제목 업데이트 (HTML에 정의된 제목 유지)
                const fileInfoElement = button.closest('.file-item').querySelector('.file-info h3');
                if (fileInfoElement && filesData[fileId].title) {
                }
            }
        });
    }
    ...
    function showErrorMessage(message) {
        ...
        errorDiv.style.cssText = 'background-color: #ffeeee; color: #cc0000; padding: 15px; border-radius: 8px; margin: 20px 0; text-align: center; border: 1px solid #ffcccc;';
    ...
    function showLoading(show = true) {
        ...
        if (show) {
            if (!loadingEl) {
                ...
                loadingEl.style.cssText = 'position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background-color: rgba(66, 133, 244, 0.9); color: white; padding: 20px; border-radius: 8px; z-index: 1000;';
